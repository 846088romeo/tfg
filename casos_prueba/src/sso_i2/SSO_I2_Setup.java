...
public class SSO_I2_Setup extends AnBx_CommandLine_Parser<SSO_I2_Roles, SSO_I2_Channels> {
	...
	public static void main(String[] args) {
		...
		SSO_I2_Setup p = new SSO_I2_Setup(args, SSO_I2_Setup.class.getSimpleName(), SSO_I2.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "SSO_I2_Setup", durationInMs));
	}

	public SSO_I2_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, SSO_I2_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		SSO_I2_Roles roleshare = getRoleShare(SSO_I2_Roles.class);
		readConfigFile(aliases, SSO_I2_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<SSO_I2_Roles> shroles, List<SSO_I2_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (SSO_I2_Roles shrole : shroles) {
	        readConfigFile(aliases, SSO_I2_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (SSO_I2_Roles parrole : parroles) {
