...
public final class SET_Orig_CommandLine_Parser extends AnBx_CommandLine_Parser<SET_Orig_Roles,SET_Orig_Channels> {

	public SET_Orig_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, SET_Orig_Roles.class, true);
	...
	protected void initProtocol() {
		...
		SET_Orig_Principal SET_Orig_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, SET_Orig_Roles.class, SET_Orig_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			SET_Orig_pr = new SET_Orig_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_C:
				SET_Orig_pr.run(new SET_Orig_ROLE_C(role, protname, sharepath),sessions);
				break;
			case ROLE_M:
				SET_Orig_pr.run(new SET_Orig_ROLE_M(role, protname, sharepath),sessions);
				break;
			case ROLE_a:
				SET_Orig_pr.run(new SET_Orig_ROLE_a(role, protname, sharepath),sessions);
				break;
