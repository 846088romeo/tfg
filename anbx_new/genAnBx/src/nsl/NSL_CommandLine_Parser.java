...
public final class NSL_CommandLine_Parser extends AnBx_CommandLine_Parser<NSL_Roles,NSL_Channels> {

	public NSL_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, NSL_Roles.class, true);
	...
	protected void initProtocol() {
		...
		NSL_Principal NSL_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, NSL_Roles.class, NSL_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			NSL_pr = new NSL_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				NSL_pr.run(new NSL_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				NSL_pr.run(new NSL_ROLE_B(role, protname, sharepath),sessions);
				break;
