...
public final class Orig_2KP_CommandLine_Parser extends AnBx_CommandLine_Parser<Orig_2KP_Roles,Orig_2KP_Channels> {

	public Orig_2KP_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, Orig_2KP_Roles.class, true);
	...
	protected void initProtocol() {
		...
		Orig_2KP_Principal Orig_2KP_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, Orig_2KP_Roles.class, Orig_2KP_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			Orig_2KP_pr = new Orig_2KP_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_C:
				Orig_2KP_pr.run(new Orig_2KP_ROLE_C(role, protname, sharepath),sessions);
				break;
			case ROLE_M:
				Orig_2KP_pr.run(new Orig_2KP_ROLE_M(role, protname, sharepath),sessions);
				break;
			case ROLE_a:
				Orig_2KP_pr.run(new Orig_2KP_ROLE_a(role, protname, sharepath),sessions);
				break;
