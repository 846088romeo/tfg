...
public final class Goss_CommandLine_Parser extends AnBx_CommandLine_Parser<Goss_Roles,Goss_Channels> {

	public Goss_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, Goss_Roles.class, true);
	...
	protected void initProtocol() {
		...
		Goss_Principal Goss_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, Goss_Roles.class, Goss_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			Goss_pr = new Goss_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				Goss_pr.run(new Goss_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				Goss_pr.run(new Goss_ROLE_B(role, protname, sharepath),sessions);
				break;
