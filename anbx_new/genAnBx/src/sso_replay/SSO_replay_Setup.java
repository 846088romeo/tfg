...
public class SSO_replay_Setup extends AnBx_CommandLine_Parser<SSO_replay_Roles, SSO_replay_Channels> {
	...
	public static void main(String[] args) {
		...
		SSO_replay_Setup p = new SSO_replay_Setup(args, SSO_replay_Setup.class.getSimpleName(), SSO_replay.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "SSO_replay_Setup", durationInMs));
	}

	public SSO_replay_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, SSO_replay_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		SSO_replay_Roles roleshare = getRoleShare(SSO_replay_Roles.class);
		readConfigFile(aliases, SSO_replay_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<SSO_replay_Roles> shroles, List<SSO_replay_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (SSO_replay_Roles shrole : shroles) {
	        readConfigFile(aliases, SSO_replay_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (SSO_replay_Roles parrole : parroles) {
