...
public class IKEv2DS_Setup extends AnBx_CommandLine_Parser<IKEv2DS_Roles, IKEv2DS_Channels> {
	...
	public static void main(String[] args) {
		...
		IKEv2DS_Setup p = new IKEv2DS_Setup(args, IKEv2DS_Setup.class.getSimpleName(), IKEv2DS.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "IKEv2DS_Setup", durationInMs));
	}

	public IKEv2DS_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, IKEv2DS_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		IKEv2DS_Roles roleshare = getRoleShare(IKEv2DS_Roles.class);
		readConfigFile(aliases, IKEv2DS_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<IKEv2DS_Roles> shroles, List<IKEv2DS_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (IKEv2DS_Roles shrole : shroles) {
	        readConfigFile(aliases, IKEv2DS_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (IKEv2DS_Roles parrole : parroles) {
