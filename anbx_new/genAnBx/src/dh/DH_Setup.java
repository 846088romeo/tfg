...
public class DH_Setup extends AnBx_CommandLine_Parser<DH_Roles, DH_Channels> {
	...
	public static void main(String[] args) {
		...
		DH_Setup p = new DH_Setup(args, DH_Setup.class.getSimpleName(), DH.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "DH_Setup", durationInMs));
	}

	public DH_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, DH_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		DH_Roles roleshare = getRoleShare(DH_Roles.class);
		readConfigFile(aliases, DH_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<DH_Roles> shroles, List<DH_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (DH_Roles shrole : shroles) {
	        readConfigFile(aliases, DH_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (DH_Roles parrole : parroles) {
