...
public final class WMF_CommandLine_Parser extends AnBx_CommandLine_Parser<WMF_Roles,WMF_Channels> {

	public WMF_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, WMF_Roles.class, true);
	...
	protected void initProtocol() {
		...
		WMF_Principal WMF_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, WMF_Roles.class, WMF_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			WMF_pr = new WMF_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				WMF_pr.run(new WMF_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_s:
				WMF_pr.run(new WMF_ROLE_s(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				WMF_pr.run(new WMF_ROLE_B(role, protname, sharepath),sessions);
				break;
