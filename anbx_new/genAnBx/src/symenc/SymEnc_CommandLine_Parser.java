...
public final class SymEnc_CommandLine_Parser extends AnBx_CommandLine_Parser<SymEnc_Roles,SymEnc_Channels> {

	public SymEnc_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, SymEnc_Roles.class, true);
	...
	protected void initProtocol() {
		...
		SymEnc_Principal SymEnc_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, SymEnc_Roles.class, SymEnc_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			SymEnc_pr = new SymEnc_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				SymEnc_pr.run(new SymEnc_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				SymEnc_pr.run(new SymEnc_ROLE_B(role, protname, sharepath),sessions);
				break;
