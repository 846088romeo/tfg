...
public final class DH_auth_CommandLine_Parser extends AnBx_CommandLine_Parser<DH_auth_Roles,DH_auth_Channels> {

	public DH_auth_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, DH_auth_Roles.class, true);
	...
	protected void initProtocol() {
		...
		DH_auth_Principal DH_auth_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, DH_auth_Roles.class, DH_auth_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			DH_auth_pr = new DH_auth_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				DH_auth_pr.run(new DH_auth_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				DH_auth_pr.run(new DH_auth_ROLE_B(role, protname, sharepath),sessions);
				break;
