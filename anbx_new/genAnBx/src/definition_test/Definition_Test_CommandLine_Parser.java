...
public final class Definition_Test_CommandLine_Parser extends AnBx_CommandLine_Parser<Definition_Test_Roles,Definition_Test_Channels> {

	public Definition_Test_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, Definition_Test_Roles.class, true);
	...
	protected void initProtocol() {
		...
		Definition_Test_Principal Definition_Test_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, Definition_Test_Roles.class, Definition_Test_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			Definition_Test_pr = new Definition_Test_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				Definition_Test_pr.run(new Definition_Test_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				Definition_Test_pr.run(new Definition_Test_ROLE_B(role, protname, sharepath),sessions);
				break;
