...
public class Secret_Pair_Setup extends AnBx_CommandLine_Parser<Secret_Pair_Roles, Secret_Pair_Channels> {
	...
	public static void main(String[] args) {
		...
		Secret_Pair_Setup p = new Secret_Pair_Setup(args, Secret_Pair_Setup.class.getSimpleName(), Secret_Pair.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "Secret_Pair_Setup", durationInMs));
	}

	public Secret_Pair_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, Secret_Pair_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		Secret_Pair_Roles roleshare = getRoleShare(Secret_Pair_Roles.class);
		readConfigFile(aliases, Secret_Pair_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<Secret_Pair_Roles> shroles, List<Secret_Pair_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (Secret_Pair_Roles shrole : shroles) {
	        readConfigFile(aliases, Secret_Pair_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (Secret_Pair_Roles parrole : parroles) {
