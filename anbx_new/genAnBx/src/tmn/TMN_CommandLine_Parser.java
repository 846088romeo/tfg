...
public final class TMN_CommandLine_Parser extends AnBx_CommandLine_Parser<TMN_Roles,TMN_Channels> {

	public TMN_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, TMN_Roles.class, true);
	...
	protected void initProtocol() {
		...
		TMN_Principal TMN_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, TMN_Roles.class, TMN_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			TMN_pr = new TMN_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				TMN_pr.run(new TMN_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_s:
				TMN_pr.run(new TMN_ROLE_s(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				TMN_pr.run(new TMN_ROLE_B(role, protname, sharepath),sessions);
				break;
