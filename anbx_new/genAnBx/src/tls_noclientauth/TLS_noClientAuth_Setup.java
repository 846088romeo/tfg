...
public class TLS_noClientAuth_Setup extends AnBx_CommandLine_Parser<TLS_noClientAuth_Roles, TLS_noClientAuth_Channels> {
	...
	public static void main(String[] args) {
		...
		TLS_noClientAuth_Setup p = new TLS_noClientAuth_Setup(args, TLS_noClientAuth_Setup.class.getSimpleName(), TLS_noClientAuth.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "TLS_noClientAuth_Setup", durationInMs));
	}

	public TLS_noClientAuth_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, TLS_noClientAuth_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		TLS_noClientAuth_Roles roleshare = getRoleShare(TLS_noClientAuth_Roles.class);
		readConfigFile(aliases, TLS_noClientAuth_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<TLS_noClientAuth_Roles> shroles, List<TLS_noClientAuth_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (TLS_noClientAuth_Roles shrole : shroles) {
	        readConfigFile(aliases, TLS_noClientAuth_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (TLS_noClientAuth_Roles parrole : parroles) {
