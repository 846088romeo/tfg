...
public class PK_Lookup_Setup extends AnBx_CommandLine_Parser<PK_Lookup_Roles, PK_Lookup_Channels> {
	...
	public static void main(String[] args) {
		...
		PK_Lookup_Setup p = new PK_Lookup_Setup(args, PK_Lookup_Setup.class.getSimpleName(), PK_Lookup.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "PK_Lookup_Setup", durationInMs));
	}

	public PK_Lookup_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, PK_Lookup_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		PK_Lookup_Roles roleshare = getRoleShare(PK_Lookup_Roles.class);
		readConfigFile(aliases, PK_Lookup_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<PK_Lookup_Roles> shroles, List<PK_Lookup_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (PK_Lookup_Roles shrole : shroles) {
	        readConfigFile(aliases, PK_Lookup_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (PK_Lookup_Roles parrole : parroles) {
