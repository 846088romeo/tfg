...
public class TagTT_Setup extends AnBx_CommandLine_Parser<TagTT_Roles, TagTT_Channels> {
	...
	public static void main(String[] args) {
		...
		TagTT_Setup p = new TagTT_Setup(args, TagTT_Setup.class.getSimpleName(), TagTT.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "TagTT_Setup", durationInMs));
	}

	public TagTT_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, TagTT_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		TagTT_Roles roleshare = getRoleShare(TagTT_Roles.class);
		readConfigFile(aliases, TagTT_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<TagTT_Roles> shroles, List<TagTT_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (TagTT_Roles shrole : shroles) {
	        readConfigFile(aliases, TagTT_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (TagTT_Roles parrole : parroles) {
