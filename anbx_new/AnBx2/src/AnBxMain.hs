...
mkAnBxActionExpandBullet ka certified options a@((peer1@(id1,_,_),AnBxAst.Sharing SHAgree,peer2@(id2,_,_)),msgw,msg1,msg2) = if anbxexpandagree options then -- expands agree action for attack trace reconstruction
                                                                                                                                let -- expansion with agreed sharing function
                                                                                                                                    ...
                                                                                                                                    msgAgreed = wrap (agreedSecureMsg msg ids) -- agreed message
...
trAnB :: AnBxProtocol -> AnBxOnP -> Protocol
trAnB protocol opt =
    let ((protocolname,prottype),types,_,equations,knowledge,shares,abstraction,actions,goals) = protocol
        out = anbxouttype opt
        -- no declarations beyond this point. They should have been already been processed at this stage
        ...
        prot5 = if anbexeccheck opt && isOutAnB out then -- only for AnB-like output, for other output this check is done anyway at a later stage
                                        let
                                        !_ =  trProt2Execnarr prot4 Nothing opt -- executability check including equations
                                        in prot4
                                else prot4
    in prot5
