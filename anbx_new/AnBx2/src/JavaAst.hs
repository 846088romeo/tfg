...
data JAction = -- int=step string=agent
       ...
       | JEmitReplay (Int,String,JChannel,NExpression,NExpression) -- step,agent,channel,dest expression,message
...
showJChannelName :: JChannel -> String
showJChannelName (_,_,_,_,_,_,name) = name
...
showJAction (JEmitReplay (step,a,ch,e,f)) = a ++ "|" ++ show step ++": sendReplay(" ++  show (typeof e) ++ "," ++ show (typeof f) ++ ") \t # ch: " ++ show ch
...
mkProt2J prot@(_,ptypes,_,_,_,_,_,_,_) intrProtInfos options cfg = let
                                                                    ...
                                                                    (jactions,jsteps0) = trace ("mkProt2J WFF Final Count: " ++ show (length [act | act@(NACheck (_,_,phi)) <- execnarr, case phi of {FSingle (FWff _) -> True; _ -> False}]) ++ " for " ++ show (anbxouttype options)) $ mapActions execnarr ctx channels 0 (synthesistypeenc options)
                                                                    jsteps = sort jsteps0
