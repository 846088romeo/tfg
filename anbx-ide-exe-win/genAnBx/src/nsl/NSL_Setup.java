...
public class NSL_Setup extends AnBx_CommandLine_Parser<NSL_Roles, NSL_Channels> {
	...
	public static void main(String[] args) {
		...
		NSL_Setup p = new NSL_Setup(args, NSL_Setup.class.getSimpleName(), NSL.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "NSL_Setup", durationInMs));
	}

	public NSL_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, NSL_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		NSL_Roles roleshare = getRoleShare(NSL_Roles.class);
		readConfigFile(aliases, NSL_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<NSL_Roles> shroles, List<NSL_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (NSL_Roles shrole : shroles) {
	        readConfigFile(aliases, NSL_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (NSL_Roles parrole : parroles) {
