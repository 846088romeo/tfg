...
public final class TLS_noClientAuth_CommandLine_Parser extends AnBx_CommandLine_Parser<TLS_noClientAuth_Roles,TLS_noClientAuth_Channels> {

	public TLS_noClientAuth_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, TLS_noClientAuth_Roles.class, true);
	...
	protected void initProtocol() {
		...
		TLS_noClientAuth_Principal TLS_noClientAuth_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, TLS_noClientAuth_Roles.class, TLS_noClientAuth_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			TLS_noClientAuth_pr = new TLS_noClientAuth_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				TLS_noClientAuth_pr.run(new TLS_noClientAuth_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				TLS_noClientAuth_pr.run(new TLS_noClientAuth_ROLE_B(role, protname, sharepath),sessions);
				break;
