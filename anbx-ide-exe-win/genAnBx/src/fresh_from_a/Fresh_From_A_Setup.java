...
public class Fresh_From_A_Setup extends AnBx_CommandLine_Parser<Fresh_From_A_Roles, Fresh_From_A_Channels> {
	...
	public static void main(String[] args) {
		...
		Fresh_From_A_Setup p = new Fresh_From_A_Setup(args, Fresh_From_A_Setup.class.getSimpleName(), Fresh_From_A.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "Fresh_From_A_Setup", durationInMs));
	}

	public Fresh_From_A_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, Fresh_From_A_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		Fresh_From_A_Roles roleshare = getRoleShare(Fresh_From_A_Roles.class);
		readConfigFile(aliases, Fresh_From_A_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<Fresh_From_A_Roles> shroles, List<Fresh_From_A_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (Fresh_From_A_Roles shrole : shroles) {
	        readConfigFile(aliases, Fresh_From_A_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (Fresh_From_A_Roles parrole : parroles) {
