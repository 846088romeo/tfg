...
public final class Forward_Blind_A_Digest_ROLE_A extends AnB_Protocol<Forward_Blind_A_Digest_Steps,Forward_Blind_A_Digest_Roles> {
...
//	private static boolean loop = false;
	...
	private SecretKey Hx1CMsg = null;
	
	
	public Forward_Blind_A_Digest_ROLE_A(Forward_Blind_A_Digest_Roles role, String name, String sharepath) {
		super();
		this.role = role;
		this.name = name;   
	...
	public void run(Map<String, AnB_Session> lbs, Map<String, String> aliases, long sessions) {
		...
		this.lbs = lbs;
		Forward_Blind_A_Digest_ROLE_A.sessions = sessions;
		...
		do {
			...
			try {
                    ...
                    executeStep(ROLE_A_channel_ROLE_B_Client_Insecure, Forward_Blind_A_Digest_Steps.STEP_0);
	...
	protected void executeStep(AnB_Session s, Forward_Blind_A_Digest_Steps step) {
		...
		case STEP_0:
			...
			// 	A -> B, (A|C|C): [Msg,[[Msg:C]]]
			Msg = s.getNonce();
			Hx1CMsg = s.getHmacKey();
			s.Send(s.encrypt(s.sign(new AnBx_Params(aliases.get("ROLE_C"),s.makeDigest(new AnBx_Params(Msg,s.makeDigest(s.makeHmac(Msg,(SecretKey) Hx1CMsg)))))),aliases.get("ROLE_C")));
