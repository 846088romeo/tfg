...
public class TLS_Setup extends AnBx_CommandLine_Parser<TLS_Roles, TLS_Channels> {
	...
	public static void main(String[] args) {
		...
		TLS_Setup p = new TLS_Setup(args, TLS_Setup.class.getSimpleName(), TLS.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "TLS_Setup", durationInMs));
	}

	public TLS_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, TLS_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		TLS_Roles roleshare = getRoleShare(TLS_Roles.class);
		readConfigFile(aliases, TLS_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<TLS_Roles> shroles, List<TLS_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (TLS_Roles shrole : shroles) {
	        readConfigFile(aliases, TLS_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (TLS_Roles parrole : parroles) {
