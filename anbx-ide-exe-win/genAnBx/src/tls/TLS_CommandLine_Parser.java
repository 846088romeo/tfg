...
public final class TLS_CommandLine_Parser extends AnBx_CommandLine_Parser<TLS_Roles,TLS_Channels> {

	public TLS_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, TLS_Roles.class, true);
	...
	protected void initProtocol() {
		...
		TLS_Principal TLS_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, TLS_Roles.class, TLS_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			TLS_pr = new TLS_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_s:
				TLS_pr.run(new TLS_ROLE_s(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				TLS_pr.run(new TLS_ROLE_B(role, protname, sharepath),sessions);
				break;
			case ROLE_A:
				TLS_pr.run(new TLS_ROLE_A(role, protname, sharepath),sessions);
				break;
