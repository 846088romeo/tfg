...
public class Latex_Test_Setup extends AnBx_CommandLine_Parser<Latex_Test_Roles, Latex_Test_Channels> {
	...
	public static void main(String[] args) {
		...
		Latex_Test_Setup p = new Latex_Test_Setup(args, Latex_Test_Setup.class.getSimpleName(), Latex_Test.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "Latex_Test_Setup", durationInMs));
	}

	public Latex_Test_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, Latex_Test_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		Latex_Test_Roles roleshare = getRoleShare(Latex_Test_Roles.class);
		readConfigFile(aliases, Latex_Test_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<Latex_Test_Roles> shroles, List<Latex_Test_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (Latex_Test_Roles shrole : shroles) {
	        readConfigFile(aliases, Latex_Test_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (Latex_Test_Roles parrole : parroles) {
