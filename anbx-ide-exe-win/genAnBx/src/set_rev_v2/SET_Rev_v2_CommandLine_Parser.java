...
public final class SET_Rev_v2_CommandLine_Parser extends AnBx_CommandLine_Parser<SET_Rev_v2_Roles,SET_Rev_v2_Channels> {

	public SET_Rev_v2_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, SET_Rev_v2_Roles.class, true);
	...
	protected void initProtocol() {
		...
		SET_Rev_v2_Principal SET_Rev_v2_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, SET_Rev_v2_Roles.class, SET_Rev_v2_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			SET_Rev_v2_pr = new SET_Rev_v2_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_C:
				SET_Rev_v2_pr.run(new SET_Rev_v2_ROLE_C(role, protname, sharepath),sessions);
				break;
			case ROLE_M:
				SET_Rev_v2_pr.run(new SET_Rev_v2_ROLE_M(role, protname, sharepath),sessions);
				break;
			case ROLE_a:
				SET_Rev_v2_pr.run(new SET_Rev_v2_ROLE_a(role, protname, sharepath),sessions);
				break;
