...
public final class DH_hmac_CommandLine_Parser extends AnBx_CommandLine_Parser<DH_hmac_Roles,DH_hmac_Channels> {

	public DH_hmac_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, DH_hmac_Roles.class, true);
	...
	protected void initProtocol() {
		...
		DH_hmac_Principal DH_hmac_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, DH_hmac_Roles.class, DH_hmac_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			DH_hmac_pr = new DH_hmac_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				DH_hmac_pr.run(new DH_hmac_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				DH_hmac_pr.run(new DH_hmac_ROLE_B(role, protname, sharepath),sessions);
				break;
