...
public final class SSO_intr_CommandLine_Parser extends AnBx_CommandLine_Parser<SSO_intr_Roles,SSO_intr_Channels> {

	public SSO_intr_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, SSO_intr_Roles.class, true);
	...
	protected void initProtocol() {
		...
		SSO_intr_Principal SSO_intr_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, SSO_intr_Roles.class, SSO_intr_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			SSO_intr_pr = new SSO_intr_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_C:
				SSO_intr_pr.run(new SSO_intr_ROLE_C(role, protname, sharepath),sessions);
				break;
			case ROLE_SP:
				SSO_intr_pr.run(new SSO_intr_ROLE_SP(role, protname, sharepath),sessions);
				break;
			case ROLE_idp:
				SSO_intr_pr.run(new SSO_intr_ROLE_idp(role, protname, sharepath),sessions);
				break;
			case ROLE_intr:
				SSO_intr_pr.run(new SSO_intr_ROLE_intr(role, protname, sharepath),sessions);
				break;
