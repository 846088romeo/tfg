...
public class SSO_intr_Setup extends AnBx_CommandLine_Parser<SSO_intr_Roles, SSO_intr_Channels> {
	...
	public static void main(String[] args) {
		...
		SSO_intr_Setup p = new SSO_intr_Setup(args, SSO_intr_Setup.class.getSimpleName(), SSO_intr.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "SSO_intr_Setup", durationInMs));
	}

	public SSO_intr_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, SSO_intr_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		SSO_intr_Roles roleshare = getRoleShare(SSO_intr_Roles.class);
		readConfigFile(aliases, SSO_intr_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<SSO_intr_Roles> shroles, List<SSO_intr_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (SSO_intr_Roles shrole : shroles) {
	        readConfigFile(aliases, SSO_intr_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (SSO_intr_Roles parrole : parroles) {
