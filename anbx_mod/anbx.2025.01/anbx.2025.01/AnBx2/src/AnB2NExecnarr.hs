...
trProt2Execnarr :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> IO Execnarr
trProt2Execnarr origprot trImpsAndProt options = case trImpsAndProt of
  ...
  (decl, ex, ctx2) <- case intrProt of
    Just (_,(trname,trtypes,trdefs,treqs,trkn,trsh,trabs,tracts,goals),trActsIdx,intrMsgToPrint) -> do
      ...
      let declsIntr = filter (\decl -> case decl of
                                    ...
                                    DShare (_,_,_,nags) -> intrName `elem` [ id | (_,id) <- nags]
                          ) trDecls
...
dbgExecnarr :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> IO String
dbgExecnarr origprot trImpsAndProt options = do
  let prot = case trImpsAndProt of
              Just (_,trprot,_,_) -> trprot
              Nothing -> origprot
  execNarr <- trProt2Execnarr origprot trImpsAndProt options
  return $ "(* Protocol: " ++ protName prot ++ " *)\n\n" ++ showExecnarr execNarr

dbgNExecnarr :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> IO String
dbgNExecnarr prot trProtData options = do
  nexecnarr <- trProt2NExecnarr prot trProtData options
  return $ showNExecnarr nexecnarr

dbgSpyerSPI :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> IO String
dbgSpyerSPI prot trImpsAndProt options = do
  execnarr <- execnarrOfProt prot trImpsAndProt options
  return $ showSpiOfExecnarr (protName prot) execnarr

dbgKnowExecnarr :: Protocol -> AnBxOnP -> IO String
dbgKnowExecnarr prot options = do
  kappa <- knowOfProt prot options
  return $ printMapSK kappa ++ "\n"