...
compileAnB2ExecnarrKnow context@(next_var,_,_,_) (ctx,types,sh,[],decl,equations,((_,ActionComment _ s,_),_,_,_):xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache = do
    (xnarr,newKappa,newMapGoals) <- compileAnB2ExecnarrKnow context (ctx,types,sh,[],decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache
    return (NAComment (next_var,s):xnarr,newKappa,newMapGoals)

-- knowledge/declarations
compileAnB2ExecnarrKnow (next_var,privnames,kappa,gennames) (ctx,types,sh,DKnow ((_,a),m):ds,decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache =
               let s = Set.intersection (namesOfNExpression m) gennames in
                        if Set.null s then
                                ...
                                in compileAnB2ExecnarrKnow (next_var,privnames,newKappa,gennames) (ctx,types,sh,ds,decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache
                        else error ("cannot compile AnB to ExecNarr - message " ++ show m ++ " contains generated names, for example: " ++ setChoose s)

compileAnB2ExecnarrKnow (next_var,privnames,kappa,gennames) (ctx,types,sh,DGenerates ((_,a),n):ds,decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache =
                let
                ...
                in if not (Set.member ng all_names) then
                    ...
                    in do
                        (xnarr,newKappa1,newMapGoals) <- compileAnB2ExecnarrKnow (next_var,privnames,newKappa,gennames1) (ctx,types,sh,ds,decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache
...
compileAnB2ExecnarrKnow (next_var,_,kappa,_) (ctx,types,sh,[],_,equations,[],g:gs,_,_) mapgoals evn opt out replayCache = 
    let (msg,agentList) = msgAgentsOfGoal g
...
compileAnB2ExecnarrKnow (next_var,_,kappa,_) (ctx,types,sh,[],_,equations,[],[],g:goalsR,_) mapgoals evn opt out replayCache = 
    let (msg,agentList) = msgAgentsOfGoal g
        -- check all agents can synthetise the message
...
compileAnB2ExecnarrKnow context@(next_var,privnames,kappa,gennames) (ctx,types,sh,[],decl,equations,x@(((a,_,_),channeltype,(b,_,_)),msg,_,_):xs,goalsS,goalsR,seenSQN) mapgoals evn@(_,endEvnrAuth,endEvnrSecr) opt out replayCache
                        ...
                        | channeltype == Sharing SHShare || ((channeltype == Sharing SHAgree || channeltype == Sharing SHAgreeInsecurely) && not (isOutTypePV out)) = compileAnB2ExecnarrKnow context (ctx,types,sh,[],decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache
                        -- standard actions
                        | otherwise =
                        if next_var==0 && msg == Atom syncMsg then
                            -- skip first empty action if any
                            compileAnB2ExecnarrKnow context (ctx,types,sh,[],decl,equations,xs,goalsS,goalsR,seenSQN) mapgoals evn opt out replayCache
                        else do
                            let m = trMsg msg ctx
                            let ka0 = getKappa kappa a ctx
                            let ka = analysisStepEq ka0 equations ctx opt
                            case (inSynthesis ka (agent2NExpression b ctx) equations, inSynthesis ka m equations ctx opt) of
                                (_, Nothing) -> error ("agent " ++ a ++ " cannot compile AnB to ExecNarr - in action: " ++ showAction x ++ "\n" ++ errorInSynthesis a m ka equations)
                                (_, Just em) -> do
                                    let xvar = newVar next_var
                                    let kappa1 = updateKappa kappa a ka
                                    let t = typeofTS m ctx
                                    let (kb, phi) = addKnowledge (m, NEVar (t, xvar) em) (getKappa kappa1 b ctx) equations ctx opt
                                    let newKappa = updateKappa kappa1 b kb
                                    let mapgoals1 = updateMapGoalsList next_var newKappa mapgoals goalsS ctx equations opt
                                    let ((gS1, gS2), (gR1, gR2)) = goals2filter a b goalsS goalsR newKappa ctx equations opt
                                    let acts_be = concatMap (\x -> beginEvents next_var endEvnrSecr newKappa x ctx equations mapgoals opt) gS1
                                    let acts_ee = concatMap (\x -> endEvents endEvnrAuth next_var newKappa x ctx equations mapgoals opt RelaxKnowAgentFalse) gR1

                                    let na = agent2NEIdent a ctx
                                    let nb = agent2NEIdent b ctx

                                    -- Replay logic
                                    let replayCache' = updateReplayCache msg replayCache
                                    msgToSend <- chooseReplay replayCache msg
                                    let emSend = trMsg msgToSend ctx
                                    let emRecv = trMsg msg ctx

                                    let acts1 =  NAEmit (next_var, a, (na, channeltype, nb), agent2NExpression b ctx, emSend)
                                            : [NAReceive (next_var, b, (nb, channeltype, na), NEVar (t, xvar) emRecv)]

                                    let (acts2, seenSQN1) = seenEvents next_var b newKappa ctx equations seenSQN decl opt
                                    let acts3 = [NACheck (next_var, b, phi)]

                                    (acts4, newKappa1, newMapGoals) <- compileAnB2ExecnarrKnow (next_var + 1, privnames, newKappa, gennames)
                                                                    (ctx, types, sh, [], decl, equations, xs, gS2, gR2, seenSQN1)
                                                                    mapgoals1 evn opt out replayCache'

                                    return (acts_be ++ acts1 ++ acts2 ++ acts3 ++ acts4 ++ acts_ee, newKappa1, newMapGoals)
...
execnarrKnowOfProt :: Protocol -> AnBxOnP -> IO (Execnarr,MapSK,MapGoals)
-- execnarrKnowOfProt (_,_,(_,_),_,_,_,actions,goals) _ | trace  ("\nexecnarrKnowOfProt\n\ta: " ++ show (head actions) ++ "\n\tgoals: " ++ showSimpleGoals goals) False = undefined
execnarrKnowOfProt prot@(_,types,_,anbequations,(_,wh),shares,_,actions,goals) anbxopt = do
                        let
                        ...
                        (narr, kappa, mapgoals) <-
                            compileAnB2ExecnarrKnow
                                (0, Set.empty, Map.empty, Set.empty)
                                (ctx, types, shares, decs, decs, equations, actions, goals2ver, goals2ver, [])
                                newmapgoals evn anbxopt out replayCache0

                        let xnarr = nubOrd . sortExecNarr . cleanExecNarr $ (wh_actions ctx ++ narr)
                        return (xnarr, kappa, mapgoals)


-- compute execnarr
execnarrOfProt :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> IO Execnarr
...
execnarrOfProt prot trImpsAndProt anbxopt = do
...
knowOfProt :: Protocol -> AnBxOnP -> IO MapSK
knowOfProt prot anbxopt = do
  (_, kappa, _) <- execnarrKnowOfProt prot anbxopt
  return kappa

-- compute mapGoals (agent,goalID) -> step 
mapgoalsOfProt :: Protocol -> AnBxOnP -> IO MapGoals
mapgoalsOfProt prot anbxopt = do
  (_, _, mapgoals) <- execnarrKnowOfProt prot anbxopt
  return mapgoals
----------------------------------------------------------------------------------------------------------
-- attack trace reconstruction functions

mergeHonestAndTraceExecNarrs :: Execnarr -> Protocol -> [Int] -> Maybe Msg -> AnBxOnP -> IO Execnarr
mergeHonestAndTraceExecNarrs passiveIntrNarr (trname,trtypes,trdefs,treqs,trkn,trsh,trabs,tracts,trgoals) trActsIdx intrMsgToPrint options = do
  let activeAgs = Set.fromList (getActiveAgents tracts)
  let intrName = anbxmitm options
  let alignedTrProt = (trname,trtypes,trdefs,treqs,trkn,trsh,trabs,alignTraceActionsWithPassive tracts trActsIdx beginEvnrDefault intrName,trgoals)
  let ctx = buildJContext alignedTrProt

  -- Llamada en IO
  passiveNarr <- execnarrOfProt alignedTrProt Nothing (options { nogoals = True })

  let narrWithIntrActions = injectIntrActionsInPassiveExecNarr passiveIntrNarr passiveNarr trActsIdx activeAgs intrName
  let equations = trEquations treqs trtypes ctx

  narrWithAddedIntrWff <- case intrMsgToPrint of
    Just toPrint -> do
      let m = trMsg toPrint ctx
      kappa <- knowOfProt alignedTrProt options
      let ka0 = getKappa kappa intrName ctx
      let ka = analysisStepEq ka0 equations ctx options
      case inSynthesis ka m equations ctx options of
        ...
        Just em -> case find (\a -> case a of
                      ...
                      Just _ -> return narrWithIntrActions
                      Nothing -> case trActsIdx of
                          [] -> error "mergeHonestAndTraceExecNarrs: trace intruder action indexes list is empty"
                          _ -> return (narrWithIntrActions ++ [NACheck (last trActsIdx,intrName,FSingle (FWff em))])
    Nothing -> return narrWithIntrActions

  return narrWithAddedIntrWff
...
protAttackTraceGoals :: Protocol -> String -> AnBxOnP -> IO Protocol
protAttackTraceGoals prot@(name,types,definitions,anbequations,knowledge,shares,abstractions,actions,goals) intr opt = do
  let ctx = buildJContext prot
  let equations = trEquations anbequations types ctx
  let prot1 = (name,types,definitions,anbequations,knowledge,shares,abstractions,actions,[])

  -- Llamada en IO
  kappa <- knowOfProt prot1 opt

  let newGoals = map (\x -> goalAttackTrace x kappa intr ctx equations opt) goals
  let prot2 = (name,types,definitions,anbequations,knowledge,shares,abstractions,actions,newGoals)
  return prot2
