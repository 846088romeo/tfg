...
mkProt2J :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> AnBxCfg -> IO JProtocol
mkProt2J prot@(_,ptypes,_,_,_,_,_,_,_) intrProtInfos options cfg = do
                                                                    ((name,declarations,jequations),execnarr) <- trProt2NExecnarr prot intrProtInfos options
                                                                    let types = case intrProtInfos of
                                                                ...
                                                                return (name,jcustomtypes,jconstants,shares,agree,jequations,roles,droles,inactiveagents,jsteps,channels,fields,methods,jactions)
...
dbgJava :: Protocol -> OFMCAttackImpersonationsAndProt -> AnBxOnP -> AnBxCfg -> IO String
dbgJava origprot trImpsAndProt options cfg = do
    ...
    let (_, _, _, _, _, _, _, actions, _) = case trImpsAndProt of
