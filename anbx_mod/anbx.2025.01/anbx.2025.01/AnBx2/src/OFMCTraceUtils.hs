...
rewriteGoals :: Actions -> Protocol -> AnBxOnP -> IO (Goals, Maybe Msg)
rewriteGoals passiveActs trProt@(_, types, _, anbequations, _, _, _, _, [goal]) anbxopt = do
  let ctx1 = buildJContext trProt
  kappa <- knowOfProt trProt anbxopt
  let equations = trEquations anbequations types ctx1
  let (newgoal, intrMsgToPrint) = rewriteGoalMsg goal passiveActs trProt kappa equations ctx1 anbxopt
  return ([newgoal], intrMsgToPrint)
