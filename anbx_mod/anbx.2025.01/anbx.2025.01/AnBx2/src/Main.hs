                                        ...
                                        return (show AANB ++ " implementation can only be used for AnB file generation")
                                         ...
                                         traceInfo <- case ofmctracefilename anbxonp of
                                                        Just tracefile -> do
                                                            ...
                                                            let (anbxTrace,wpassintr,imps,trActsIndexes,intrMsgToPrint) =
                                                            ...
                                                            trProt <- compileAnBx trFileName anbxonp{anbexeccheck=False} cfg anbxTrace Nothing
                                                            return (Just (trProt, trFileName, wpassintr, imps, trActsIndexes, intrMsgToPrint))
...
getAnBxOut filename anbxonp prot cfg = do
  let prot1 = case anbxouttype anbxonp of
                AnBxLatex -> prot
...
codegen filename anbxonp anbxprot impsAndTrProt cfg = do
  ...
  case agent anbxonp of
    Just a ->
      if elem a agents
        then dbgJavaCode prot anbxonp cfg impsAndTrProt
        else error ("agent " ++ a ++ " does not exist!")
    Nothing -> dbgJavaCode prot anbxonp cfg impsAndTrProt
...
reconstructAttackTrace :: String -> String -> AnBxOnP -> AnBxCfg -> AnBxProtocol -> IO (AnBxProtocol,AnBxProtocol,SubjectiveImpersonations,[Int],Maybe Msg)
reconstructAttackTrace filename trace anbxonp cfg anbxprot = do
  let intrName = anbxmitm anbxonp
  let withIntrDecl = addIntrDecl anbxprot intrName
  compiledProt <- compileAnBx filename anbxonp{anbexeccheck=False} cfg withIntrDecl Nothing
  let wPassiveIntr@(_,_,_,_,anbknow,_,_,_,_) = splitWithIntrAnB compiledProt intrName
  let (trProt,trHonestknowSec,trActsIndexes,intrMsgToPrint) = getNamedTrace trace wPassiveIntr (snd (fst wPassiveIntr)) anbxonp
  return (trAnB2AnBx (Just (snd (fst wPassiveIntr))) trProt,
          trAnB2AnBx (Just (snd (fst wPassiveIntr))) wPassiveIntr,
          getSubjectiveImpersonations anbknow trHonestknowSec intrName,
          trActsIndexes, intrMsgToPrint)
...
file2IntermediateFormats filename anbxonp outtype anbxprot cfg trProtData = do
    prot <- compileAnBx filename anbxonp cfg anbxprot trProtData
    let
        basename = dropExtension filename
        basename0 = case outprotsuffix anbxonp of -- add suffix to file name if any is specified
                     Just s  -> basename ++ s
                     Nothing -> basename
    ...
    case outtype of
        ...
        _ -> if isOutTypePV outtype
               then anb2IntermediateFormats prot basename0 anbxonp outtype cfg outfile trProtData
...
printProtocol prot outtype options cfg outfile trProtData
  | isOutTypeNoSaveFile outtype = do
  ...
  | otherwise = do
      output <- getOut prot outtype options cfg trProtData
      let showStdOut = not (silentStdOut options)
      writeOutputFile outfile outtype output showStdOut
...
compileAnBx :: String -> AnBxOnP -> AnBxCfg -> AnBxProtocol -> OFMCAttackImpersonationsAndProt -> IO Protocol
compileAnBx filename options cfg anbxprot trProtData = do
    let ext = takeExtension filename
    let anbxprot1 = setInterpretation anbxprot ext
    let anbxprot2 = case outprotsuffix options of
                      Just s -> renameProtocol anbxprot1 (getProtName anbxprot1 ++ s)
                      Nothing -> anbxprot1
    let anbxprot3 = if passiveIntruder options
                      then mkAnBxIntr anbxprot2 options
                      else anbxprot2

    prot <- buildAnB anbxprot3 options

    let outtype = anbxouttype options
    case anbxdebugtype options of
      DNone ->
        if isExtAnB ext && (elem outtype [TypedOptExecnarr, TypedOptExecnarrDocker] || isOutTypePV outtype || isOutTypeJava outtype)
          then error ("Direct code generation from a " ++ ext ++ " file to " ++ show outtype
                      ++ " is not possible due to lack of function type signatures.\n"
                      ++ "Please rename the file to ." ++ show PTAnBx ++ ","
...
buildAnB :: AnBxProtocol -> AnBxOnP -> IO Protocol
buildAnB anbxprot options = trAnB (mkAnB anbxprot options) options

dbgPV :: Protocol -> OutType -> AnBxOnP -> AnBxCfg -> IO String
dbgPV prot pvout options cfg = do
  jprot <- mkProt2J prot Nothing options cfg
  return $ printPvtOfExecnarr jprot options pvout

getOut :: Protocol -> OutType -> AnBxOnP -> AnBxCfg -> OFMCAttackImpersonationsAndProt -> IO String
getOut protocol outtype options cfg trProtData = do
        let prot = trAnBAddPublicKeys protocol
        let showStdOut = not (silentStdOut options)
        case outtype of
            None -> error "No output was requested"
            ProtName -> return $ checkProtName prot options
            AnB -> return $ showAnB protocol
            AnBStats -> return $ (if showStdOut then showAnB protocol ++ "\n" else "") ++ showAnBStats protocol AnBStats
            AnBStatsCSV -> return $ (if showStdOut then showAnB protocol ++ "\n" else "") ++ showAnBStats protocol AnBStatsCSV
            AnBIF -> return $ mkIF protocol defaultAnBOpts{numSess = Just (ifsessions options), eqnoexec = True, noowngoal = True} options
            AnBLatex -> return $ showLatex PTAnB protocol
            Spyer -> return $ dbgSpyer prot options
            Execnarr -> return $ dbgExecnarr prot trProtData options
            KnowExecnarr -> return $ dbgKnowExecnarr prot options
            OptExecnarr -> return $ dbgNExecnarr prot trProtData options
            ...
            SPI -> return $ dbgSpyerSPI prot trProtData options
            ...
            VDM -> return $ showVDM prot VDMIndent
