...
public class DenningSacco_Setup extends AnBx_CommandLine_Parser<DenningSacco_Roles, DenningSacco_Channels> {
	...
	public static void main(String[] args) {
		...
		DenningSacco_Setup p = new DenningSacco_Setup(args, DenningSacco_Setup.class.getSimpleName(), DenningSacco.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "DenningSacco_Setup", durationInMs));
	}

	public DenningSacco_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, DenningSacco_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		DenningSacco_Roles roleshare = getRoleShare(DenningSacco_Roles.class);
		readConfigFile(aliases, DenningSacco_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<DenningSacco_Roles> shroles, List<DenningSacco_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (DenningSacco_Roles shrole : shroles) {
	        readConfigFile(aliases, DenningSacco_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (DenningSacco_Roles parrole : parroles) {
