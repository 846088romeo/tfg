...
public class TMN_Setup extends AnBx_CommandLine_Parser<TMN_Roles, TMN_Channels> {
	...
	public static void main(String[] args) {
		...
		TMN_Setup p = new TMN_Setup(args, TMN_Setup.class.getSimpleName(), TMN.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "TMN_Setup", durationInMs));
	}

	public TMN_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, TMN_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		TMN_Roles roleshare = getRoleShare(TMN_Roles.class);
		readConfigFile(aliases, TMN_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<TMN_Roles> shroles, List<TMN_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (TMN_Roles shrole : shroles) {
	        readConfigFile(aliases, TMN_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (TMN_Roles parrole : parroles) {
