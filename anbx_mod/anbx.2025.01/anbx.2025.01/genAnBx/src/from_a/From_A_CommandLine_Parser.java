...
public final class From_A_CommandLine_Parser extends AnBx_CommandLine_Parser<From_A_Roles,From_A_Channels> {

	public From_A_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, From_A_Roles.class, true);
	...
	protected void initProtocol() {
		...
		From_A_Principal From_A_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, From_A_Roles.class, From_A_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			From_A_pr = new From_A_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				From_A_pr.run(new From_A_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				From_A_pr.run(new From_A_ROLE_B(role, protname, sharepath),sessions);
				break;
