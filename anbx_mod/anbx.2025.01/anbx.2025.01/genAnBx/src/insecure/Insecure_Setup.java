...
public class Insecure_Setup extends AnBx_CommandLine_Parser<Insecure_Roles, Insecure_Channels> {
	...
	public static void main(String[] args) {
		...
		Insecure_Setup p = new Insecure_Setup(args, Insecure_Setup.class.getSimpleName(), Insecure.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "Insecure_Setup", durationInMs));
	}

	public Insecure_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, Insecure_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		Insecure_Roles roleshare = getRoleShare(Insecure_Roles.class);
		readConfigFile(aliases, Insecure_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<Insecure_Roles> shroles, List<Insecure_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (Insecure_Roles shrole : shroles) {
	        readConfigFile(aliases, Insecure_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (Insecure_Roles parrole : parroles) {
