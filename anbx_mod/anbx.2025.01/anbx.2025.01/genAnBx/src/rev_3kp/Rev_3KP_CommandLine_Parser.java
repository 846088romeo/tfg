...
public final class Rev_3KP_CommandLine_Parser extends AnBx_CommandLine_Parser<Rev_3KP_Roles,Rev_3KP_Channels> {

	public Rev_3KP_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, Rev_3KP_Roles.class, true);
	...
	protected void initProtocol() {
		...
		Rev_3KP_Principal Rev_3KP_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, Rev_3KP_Roles.class, Rev_3KP_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			Rev_3KP_pr = new Rev_3KP_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_C:
				Rev_3KP_pr.run(new Rev_3KP_ROLE_C(role, protname, sharepath),sessions);
				break;
			case ROLE_M:
				Rev_3KP_pr.run(new Rev_3KP_ROLE_M(role, protname, sharepath),sessions);
				break;
			case ROLE_a:
				Rev_3KP_pr.run(new Rev_3KP_ROLE_a(role, protname, sharepath),sessions);
				break;
