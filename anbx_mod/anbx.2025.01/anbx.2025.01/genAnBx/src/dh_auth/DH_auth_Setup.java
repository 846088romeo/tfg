...
public class DH_auth_Setup extends AnBx_CommandLine_Parser<DH_auth_Roles, DH_auth_Channels> {
	...
	public static void main(String[] args) {
		...
		DH_auth_Setup p = new DH_auth_Setup(args, DH_auth_Setup.class.getSimpleName(), DH_auth.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "DH_auth_Setup", durationInMs));
	}

	public DH_auth_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, DH_auth_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		DH_auth_Roles roleshare = getRoleShare(DH_auth_Roles.class);
		readConfigFile(aliases, DH_auth_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<DH_auth_Roles> shroles, List<DH_auth_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (DH_auth_Roles shrole : shroles) {
	        readConfigFile(aliases, DH_auth_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (DH_auth_Roles parrole : parroles) {
