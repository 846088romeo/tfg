...
public class Goss_Setup extends AnBx_CommandLine_Parser<Goss_Roles, Goss_Channels> {
	...
	public static void main(String[] args) {
		...
		Goss_Setup p = new Goss_Setup(args, Goss_Setup.class.getSimpleName(), Goss.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "Goss_Setup", durationInMs));
	}

	public Goss_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, Goss_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		Goss_Roles roleshare = getRoleShare(Goss_Roles.class);
		readConfigFile(aliases, Goss_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<Goss_Roles> shroles, List<Goss_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (Goss_Roles shrole : shroles) {
	        readConfigFile(aliases, Goss_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (Goss_Roles parrole : parroles) {
