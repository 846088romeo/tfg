...
public final class Goss_ROLE_A extends AnB_Protocol<Goss_Steps,Goss_Roles> {
...
//	private static boolean loop = false;
	...
	public Goss_ROLE_A(Goss_Roles role, String name, String sharepath) {
		super();
		this.role = role;
		this.name = name;   
	...
	public void run(Map<String, AnB_Session> lbs, Map<String, String> aliases, long sessions) {
		...
		do {
			...
			try {
                    ...
                    executeStep(ROLE_A_channel_ROLE_B_Client_Insecure, Goss_Steps.STEP_0);
					executeStep(ROLE_A_channel_ROLE_B_Client_Insecure, Goss_Steps.STEP_1);
	...
	protected void executeStep(AnB_Session s, Goss_Steps step) {
		...
		case STEP_0:
			...
			RA = s.getNonce();
			s.Send(new AnBx_Params(aliases.get("ROLE_A"),s.encrypt(new AnBx_Params(RA,tag1),aliases.get("ROLE_B"))));
		...
		case STEP_1:
			...
			eqCheck("1.2",tag2,(Crypto_ByteArray) new AnBx_Params((AnBx_Params) s.decrypt((Crypto_SealedPair) VAR_A_R1.getValue(1))).getValue(1));
