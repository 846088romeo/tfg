...
public class Secret_for_B_Setup extends AnBx_CommandLine_Parser<Secret_for_B_Roles, Secret_for_B_Channels> {
	...
	public static void main(String[] args) {
		...
		Secret_for_B_Setup p = new Secret_for_B_Setup(args, Secret_for_B_Setup.class.getSimpleName(), Secret_for_B.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "Secret_for_B_Setup", durationInMs));
	}

	public Secret_for_B_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, Secret_for_B_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		Secret_for_B_Roles roleshare = getRoleShare(Secret_for_B_Roles.class);
		readConfigFile(aliases, Secret_for_B_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<Secret_for_B_Roles> shroles, List<Secret_for_B_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (Secret_for_B_Roles shrole : shroles) {
	        readConfigFile(aliases, Secret_for_B_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (Secret_for_B_Roles parrole : parroles) {
