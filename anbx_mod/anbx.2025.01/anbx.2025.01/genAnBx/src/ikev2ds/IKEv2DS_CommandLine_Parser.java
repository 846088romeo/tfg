...
public final class IKEv2DS_CommandLine_Parser extends AnBx_CommandLine_Parser<IKEv2DS_Roles,IKEv2DS_Channels> {

	public IKEv2DS_CommandLine_Parser(String[] args, String prot) {
		super(args, prot, prot, IKEv2DS_Roles.class, true);
	...
	protected void initProtocol() {
		...
		IKEv2DS_Principal IKEv2DS_pr = null;
        if (role == null) terminate("no or incorrect role specified");
		AnBx_Debug.setAppname(role.toString());
		initRole(ct, role, cs, aliases, IKEv2DS_Roles.class, IKEv2DS_Channels.class);
		AnBx_Debug.out(layer, "myRole: " + role);
        AnBx_Debug.out(layer, "sessions: " + sessions);

		if (myAlias != null && keypath != null && sharepath != null) {
			IKEv2DS_pr = new IKEv2DS_Principal(myAlias, keypath, cs, aliases, crypto_config);
			switch (role) {
			case ROLE_A:
				IKEv2DS_pr.run(new IKEv2DS_ROLE_A(role, protname, sharepath),sessions);
				break;
			case ROLE_B:
				IKEv2DS_pr.run(new IKEv2DS_ROLE_B(role, protname, sharepath),sessions);
				break;
