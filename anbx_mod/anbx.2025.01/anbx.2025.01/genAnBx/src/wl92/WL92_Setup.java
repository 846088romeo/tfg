...
public class WL92_Setup extends AnBx_CommandLine_Parser<WL92_Roles, WL92_Channels> {
	...
	public static void main(String[] args) {
		...
		WL92_Setup p = new WL92_Setup(args, WL92_Setup.class.getSimpleName(), WL92.class.getSimpleName(), true, false);
		p.setLayer(AnBx_Layers.APPLICATION);
		p.initProtocol();
		
		long end = System.nanoTime();
      	double durationInMs = (end - start) / 1_000_000.0;
		System.out.println(String.format("%s - Execution time was %.3f ms.", "WL92_Setup", durationInMs));
	}

	public WL92_Setup(String[] args, String prot, String configName, boolean vflag, boolean parseFullArgs) {
	 	super(args, prot, configName, WL92_Roles.class, vflag, parseFullArgs);
	}

	@Override
	protected void initProtocol() {
		Map<String, String> aliases = new HashMap<String, String>();
		WL92_Roles roleshare = getRoleShare(WL92_Roles.class);
		readConfigFile(aliases, WL92_Roles.class, roleshare);
	...
	void createSharedObject(Object obj, List<WL92_Roles> shroles, List<WL92_Roles> parroles, String filenameprefix, String ext) {
	    // Create the aliases map outside of the loop
	    Map<String, String> aliases = new HashMap<String, String>();

	    // Populate the aliases map for each role
	    for (WL92_Roles shrole : shroles) {
	        readConfigFile(aliases, WL92_Roles.class, shrole); // Pass enum type role
	        // Construct the filename using the aliases map
	        
		    StringBuilder filenameBuilder = new StringBuilder(filenameprefix);
		    if (!parroles.isEmpty()) {
		    	for (WL92_Roles parrole : parroles) {
